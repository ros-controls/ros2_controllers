  ...
  const rclcpp::Time & time, const rclcpp::Duration & /*period*/)
{
  ...
  if (get_lifecycle_node()->get_current_state().id() == State::PRIMARY_STATE_INACTIVE)
  {
    if (!is_halted)
    {
      halt();
...
CallbackReturn DiffDriveController::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  cmd_vel_timeout_ = std::chrono::milliseconds{
    static_cast<int>(lifecycle_node_->get_parameter("cmd_vel_timeout").as_double() * 1000.0)};
    ...
    velocity_command_subscriber_ = lifecycle_node_->create_subscription<Twist>(
      ...
      [this](const std::shared_ptr<Twist> msg) -> void {
        ...
        if ((msg->header.stamp.sec == 0) && (msg->header.stamp.nanosec == 0))
        {
          ...
          msg->header.stamp = lifecycle_node_->get_clock()->now();
        }
...
CallbackReturn DiffDriveController::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  RCLCPP_DEBUG(lifecycle_node_->get_logger(), "Subscriber and publisher are now active.");
  return CallbackReturn::SUCCESS;
}
