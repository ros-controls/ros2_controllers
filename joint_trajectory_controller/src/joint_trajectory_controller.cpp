...
JointTrajectoryController::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  RCLCPP_INFO_STREAM(
    logger, "Controller state will be published at " << state_publish_rate << "Hz.");
...
JointTrajectoryController::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  {
    RCLCPP_ERROR(
      node_->get_logger(), "Expected %u position command interfaces, got %u", joint_names_.size(),
      joint_position_command_interface_.size());
    return rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn::ERROR;
  }
  if (!get_ordered_interfaces(
  ...
  {
    RCLCPP_ERROR(
      node_->get_logger(), "Expected %u position state interfaces, got %u", joint_names_.size(),
  ...
  {
    RCLCPP_ERROR(
      node_->get_logger(), "Expected %u velocity state interfaces, got %u", joint_names_.size(),
...
void JointTrajectoryController::halt()
{
  ...
  std::shared_ptr<trajectory_msgs::msg::JointTrajectory> trajectory_msg)
{
  ...
  auto remap = [this](
    ...
    {
      RCLCPP_WARN(node_->get_logger(), "Invalid input size (%d) for sorting", to_remap.size());
      return to_remap;
    }
  ...
  const std::string & string_for_vector_field, size_t i, bool allow_empty) const
{
  ...
  {
    RCLCPP_ERROR(
      node_->get_logger(), "Mismatch between joint_names (%u) and %s (%u) at point #%u.",
