  ...
  const rclcpp::Time & time, const rclcpp::Duration & /*period*/)
{
  ...
  if (traj_point_active_ptr_ && (*traj_point_active_ptr_)->has_trajectory_msg())
  {
    ...
    if (!(*traj_point_active_ptr_)->is_sampled_already())
    {
      if (open_loop_control_)
      {
        (*traj_point_active_ptr_)
          ->set_point_before_trajectory_msg(lifecycle_node_->now(), last_commanded_state_);
      }
      else
      {
        (*traj_point_active_ptr_)
    ...
    const bool valid_point =
      (*traj_point_active_ptr_)
        ->sample(lifecycle_node_->now(), state_desired, start_segment_itr, end_segment_itr);
...
CallbackReturn JointTrajectoryController::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  state_interface_types_ = lifecycle_node_->get_parameter("state_interfaces").as_string_array();
  ...
  open_loop_control_ = lifecycle_node_->get_parameter("open_loop_control").get_value<bool>();
  ...
  const double action_monitor_rate =
    lifecycle_node_->get_parameter("action_monitor_rate").get_value<double>();
  ...
  action_server_ = rclcpp_action::create_server<FollowJTrajAction>(
    lifecycle_node_->get_node_base_interface(), lifecycle_node_->get_node_clock_interface(),
    lifecycle_node_->get_node_logging_interface(), lifecycle_node_->get_node_waitables_interface(),
  ...
  const trajectory_msgs::msg::JointTrajectory & trajectory) const
{
  ...
  {
    RCLCPP_ERROR(lifecycle_node_->get_logger(), "Empty joint names on incoming trajectory.");
    return false;
  }
  ...
  for (auto i = 0ul; i < trajectory.joint_names.size(); ++i)
  {
    ...
    {
      RCLCPP_ERROR(
        lifecycle_node_->get_logger(), "Incoming joint %s doesn't match the controller's joints.",
