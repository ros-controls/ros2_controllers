...
public:
   ...
   *
   */
  explicit SteeringKinematics(
    const steering_controllers_library::Params & params, size_t velocity_rolling_window_size = 10)
  : forward_kinematics_(params),
  ...
  auto calculateForwardKinematics(double linear_velocity, double angular_velocity)
  {
    return forward_kinematics_.calculate(linear_velocity, angular_velocity);
  }
  // inverse kinematics computation
  auto calculateInverseKinematics(double linear_velocity, double angular_velocity)
  {
    return inverse_kinematics_.calculateCommands(linear_velocity, angular_velocity);
  }
