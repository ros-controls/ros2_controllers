...
TEST_F(PidControllerTest, test_feedforward_mode_service)
{
  ...
  ASSERT_EQ(controller_->feedforward_mode_enabled_, false);
  ...
  ASSERT_EQ(controller_->feedforward_mode_enabled_, true);
...
TEST_F(PidControllerTest, test_update_logic_feedforward_off)
{
  ...
  EXPECT_EQ(controller_->feedforward_mode_enabled_, false);
  for (const auto & interface : controller_->reference_interfaces_)
  {
    EXPECT_TRUE(std::isnan(interface));
  }
