...
using ControllerStateMsgOdom =
  ackermann_steering_controller::AckermannSteeringController::ControllerStateMsgOdom;
using ControllerStateMsgTf =
  ackermann_steering_controller::AckermannSteeringController::ControllerStateMsgTf;
...
: public ackermann_steering_controller::AckermannSteeringController
{
  ...
  FRIEND_TEST(
    AckermannSteeringControllerTest, test_age_of_last_command_zero_reference_callback_unstamped);
...
public:
    ...
    const rclcpp_lifecycle::State & previous_state) override
  {
    ...
    if (ret == CallbackReturn::SUCCESS)
    {
      ...
      {
        ref_subscriber_wait_set_.add_subscription(ref_subscriber_unstamped_);
      }
    }
    return ret;
  }
...
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_node_unstamped_ =
      std::make_shared<rclcpp::Node>("command_publisher_unstamped");
    command_publisher_unstamped_ =
      command_publisher_node_unstamped_->create_publisher<geometry_msgs::msg::Twist>(
        "/test_ackermann_steering_controller/reference_unstamped", rclcpp::SystemDefaultsQoS());
  }
...
protected:
  ...
  void publish_commands_unstamped(
    const double & twist_linear_x = 0.45, const double & twist_angular_z = 0.0)
  {
    auto wait_for_topic = [&](const auto topic_name)
    {
      size_t wait_count = 0;
      while (command_publisher_node_unstamped_->count_subscribers(topic_name) == 0)
      {
        if (wait_count >= 5)
        {
          auto error_msg =
    ...
    wait_for_topic(command_publisher_unstamped_->get_topic_name());

    geometry_msgs::msg::Twist msg_unstamped;

    msg_unstamped.linear.x = twist_linear_x;
    msg_unstamped.linear.y = std::numeric_limits<double>::quiet_NaN();
    msg_unstamped.linear.z = std::numeric_limits<double>::quiet_NaN();
    msg_unstamped.angular.x = std::numeric_limits<double>::quiet_NaN();
    msg_unstamped.angular.y = std::numeric_limits<double>::quiet_NaN();
    msg_unstamped.angular.z = twist_angular_z;

    command_publisher_unstamped_->publish(msg_unstamped);
  }
...
protected:
  ...
  rclcpp::Node::SharedPtr command_publisher_node_unstamped_;
  rclcpp::Publisher<ControllerReferenceMsg>::SharedPtr command_publisher_;
  rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr command_publisher_unstamped_;
};
