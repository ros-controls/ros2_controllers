...
void TricycleController::halt()
{
  ...
  set_command_result &= traction_joint_[0].velocity_command.get().set_value(0.0, std::numeric_limits<unsigned int>::max());
  set_command_result &= steering_joint_[0].position_command.get().set_value(0.0, std::numeric_limits<unsigned int>::max());
