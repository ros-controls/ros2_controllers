...
* Added ``spline_upsampling`` feature that ingests positions-only action chunks and upsamples them into smooth global C2 cubic splines by solving the knot velocities, configured with the new ``spline_upsampling.enable`` and ``spline_upsampling.policy_frequency`` parameters. (`#2443 <https://github.com/ros-controls/ros2_controllers/pull/2443>`_)
