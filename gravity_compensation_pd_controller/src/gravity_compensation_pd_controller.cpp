...
GravityCompensationPDController::GravityCompensationPDController()
  ...
  , dynamics_solver_loader_("inverse_dynamics_solver", "inverse_dynamics_solver::InverseDynamicsSolver")
...
controller_interface::InterfaceConfiguration GravityCompensationPDController::state_interface_configuration() const
{
  ...
  if (params.state_interfaces_names_override.position.size() == num_joints_)
  {
    state_interfaces_config.names.insert(
      state_interfaces_config.names.end(),
      params.state_interfaces_names_override.position.begin(),
  ...
  if (params.state_interfaces_names_override.velocity.size() == num_joints_)
  {
    state_interfaces_config.names.insert(
      state_interfaces_config.names.end(),
      params.state_interfaces_names_override.velocity.begin(),
  ...
  for (std::size_t i = 0; i < params.joints.size(); ++i)
  {
    reference_interfaces.emplace_back(
      hardware_interface::CommandInterface(
        std::string(get_node()->get_name()) + "/" + params.joints[i],
        "position",
...
controller_interface::CallbackReturn GravityCompensationPDController::on_error(const rclcpp_lifecycle::State& /*previous_state*/)
{
  ...
  {
    RCLCPP_ERROR(get_node()->get_logger(), "Failed to write NaN values to command interfaces while in error state.");
    return controller_interface::CallbackReturn::ERROR;
  }
...
controller_interface::return_type GravityCompensationPDController::update_reference_from_subscribers(const rclcpp::Time& /*time*/,
...
void GravityCompensationPDController::compute_control_law_()
{
  ...
  std::copy(eigen_effort_command_.data(), eigen_effort_command_.data() + num_joints_, joint_command_.begin());
}
...
bool GravityCompensationPDController::read_state_interfaces_()
{
  ...
  for (std::size_t i = 0; i < num_joints_; i++)
  {
    std::optional<double> state_position_op = ControllerInterfaceBase::state_interfaces_[i].get_optional();
    std::optional<double> state_velocity_op = ControllerInterfaceBase::state_interfaces_[i + num_joints_].get_optional();
...
bool GravityCompensationPDController::read_joint_effort_limits_from_urdf()
{
  ...
  for (std::size_t i = 0; i < num_joints; ++i)
  {
    ...
    {
      RCLCPP_WARN(get_node()->get_logger(), "Joint '%s' does not have effort limits specified in the URDF. Setting to infinity.", joint_name.c_str());
