...
TEST_F(TricycleSteeringControllerDualTractionTest, activate_success)
{
  SetUpController();
...
TEST_F(TricycleSteeringControllerDualTractionTest, update_success)
{
  SetUpController();
...
TEST_F(TricycleSteeringControllerDualTractionTest, deactivate_success)
{
  SetUpController();
...
TEST_F(TricycleSteeringControllerDualTractionTest, reactivate_success)
{
  SetUpController();
...
TEST_F(TricycleSteeringControllerDualTractionTest, test_update_logic)
{
  SetUpController();
  ...
  EXPECT_NEAR(
    ...
    0.22222222222222224, COMMON_THRESHOLD);
  EXPECT_NEAR(
    controller_->command_interfaces_[CMD_DUAL_TRACTION_STEER_WHEEL].get_optional().value(),
...
TEST_F(TricycleSteeringControllerDualTractionTest, test_update_logic_chained)
{
  SetUpController();
  rclcpp::executors::MultiThreadedExecutor executor;
  executor.add_node(controller_->get_node()->get_node_base_interface());
  ...
  EXPECT_TRUE(controller_->ordered_exported_reference_interfaces_[0]->set_value(0.1));
  EXPECT_TRUE(controller_->ordered_exported_reference_interfaces_[1]->set_value(0.2));
...
TEST_F(TricycleSteeringControllerDualTractionTest, receive_message_and_publish_updated_status)
{
  SetUpController();
  rclcpp::executors::MultiThreadedExecutor executor;
  executor.add_node(controller_->get_node()->get_node_base_interface());
