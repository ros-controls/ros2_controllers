...
using ackermann_steering_controller::CMD_DRIVE_WHEEL;
using ackermann_steering_controller::control_mode_type;
using ackermann_steering_controller::STATE_DRIVE_RIGHT_WHEEL;
...
TEST_F(AckermannSteeringControllerTest, reactivate_success)
{
  ...
  ASSERT_EQ(controller_->on_configure(rclcpp_lifecycle::State()), NODE_SUCCESS);
  ASSERT_EQ(controller_->on_activate(rclcpp_lifecycle::State()), NODE_SUCCESS);
  ASSERT_EQ(controller_->command_interfaces_[CMD_DRIVE_WHEEL].get_value(), 101.101);
  ASSERT_EQ(controller_->on_deactivate(rclcpp_lifecycle::State()), NODE_SUCCESS);
  ASSERT_TRUE(std::isnan(controller_->command_interfaces_[CMD_DRIVE_WHEEL].get_value()));
  ASSERT_EQ(controller_->on_activate(rclcpp_lifecycle::State()), NODE_SUCCESS);
  ASSERT_TRUE(std::isnan(controller_->command_interfaces_[CMD_DRIVE_WHEEL].get_value()));
...
TEST_F(AckermannSteeringControllerTest, test_update_logic_fast)
{
  SetUpController();
  rclcpp::executors::MultiThreadedExecutor executor;
  executor.add_node(controller_->get_node()->get_node_base_interface());
...
TEST_F(AckermannSteeringControllerTest, test_update_logic_slow)
{
  SetUpController();
  rclcpp::executors::MultiThreadedExecutor executor;
  executor.add_node(controller_->get_node()->get_node_base_interface());
...
TEST_F(AckermannSteeringControllerTest, receive_message_and_publish_updated_status)
{
  ...
  EXPECT_EQ(joint_command_values_[CMD_DRIVE_WHEEL], 0.45);
