...
class AckermannSteeringControllerTest
    : public AckermannSteeringControllerFixture<
          TestableAckermannSteeringController> {};
...
TEST_F(AckermannSteeringControllerTest, all_parameters_set_configure_success) {
  ...
  // ASSERT_THAT(controller_->params_.joints,
  // testing::ElementsAreArray(joint_names_));
  // ASSERT_TRUE(controller_->params_.state_joints.empty());
  // ASSERT_THAT(controller_->state_joints_,
  // testing::ElementsAreArray(joint_names_));
...
TEST_F(AckermannSteeringControllerTest, check_exported_intefaces) {
  ...
  EXPECT_EQ(command_intefaces.names[0], controller_->params_.rear_wheel_name +
                                            "/" +
                                            hardware_interface::HW_IF_VELOCITY);
  EXPECT_EQ(command_intefaces.names[1], controller_->params_.front_steer_name +
                                            "/" +
                                            hardware_interface::HW_IF_POSITION);

  auto state_intefaces = controller_->state_interface_configuration();
  ASSERT_EQ(state_intefaces.names.size(), joint_state_values_.size());
  EXPECT_EQ(state_intefaces.names[0], controller_->params_.rear_wheel_name +
                                          "/" +
                                          hardware_interface::HW_IF_POSITION);
  EXPECT_EQ(state_intefaces.names[1], controller_->params_.front_steer_name +
                                          "/" +
                                          hardware_interface::HW_IF_POSITION);
  ...
  const std::string ref_itf_name_0 =
      std::string(controller_->get_node()->get_name()) + "/" + "linear" + "/" +
      hardware_interface::HW_IF_VELOCITY;
  EXPECT_EQ(reference_interfaces[0].get_name(), ref_itf_name_0);
  EXPECT_EQ(reference_interfaces[0].get_prefix_name(),
            controller_->get_node()->get_name());
  EXPECT_EQ(reference_interfaces[0].get_interface_name(),
            std::string("linear") + "/" + hardware_interface::HW_IF_VELOCITY);

  const std::string ref_itf_name_1 =
      std::string(controller_->get_node()->get_name()) + "/" + "angular" + "/" +
      hardware_interface::HW_IF_POSITION;
  EXPECT_EQ(reference_interfaces[1].get_name(), ref_itf_name_1);
  EXPECT_EQ(reference_interfaces[1].get_prefix_name(),
            controller_->get_node()->get_name());
  EXPECT_EQ(reference_interfaces[1].get_interface_name(),
...
TEST_F(AckermannSteeringControllerTest, reactivate_success) {
  ...
  ASSERT_EQ(controller_->on_configure(rclcpp_lifecycle::State()), NODE_SUCCESS);
  ASSERT_EQ(controller_->on_activate(rclcpp_lifecycle::State()), NODE_SUCCESS);
  ASSERT_EQ(controller_->command_interfaces_[NR_CMD_ITFS - 2].get_value(),
            101.101);
  ASSERT_EQ(controller_->on_deactivate(rclcpp_lifecycle::State()),
            NODE_SUCCESS);
  ASSERT_TRUE(std::isnan(
      controller_->command_interfaces_[NR_CMD_ITFS - 2].get_value()));
  ASSERT_EQ(controller_->on_activate(rclcpp_lifecycle::State()), NODE_SUCCESS);
  ASSERT_TRUE(std::isnan(
      controller_->command_interfaces_[NR_CMD_ITFS - 2].get_value()));
...
TEST_F(AckermannSteeringControllerTest, test_message_timeout) {
  ...
  //   EXPECT_EQ((*(controller_->input_ref_.readFromNonRT()))->joint_names.size(),
  //   joint_names_.size());
  //   EXPECT_EQ((*(controller_->input_ref_.readFromNonRT()))->joint_names[0],
  //   joint_names_[0]);
...
TEST_F(AckermannSteeringControllerTest, test_update_logic_chainable) {
  ...
  //   std::make_shared<ControllerReferenceMsg>(); msg->header.stamp =
  //   controller_->get_node()->now() - controller_->ref_timeout_ -
  //                       rclcpp::Duration::from_seconds(0.1);
  //   msg->joint_names = joint_names_;
  //   msg->displacements.resize(joint_names_.size(), TEST_DISPLACEMENT);
  //   msg->velocities.resize(joint_names_.size(),
  //   std::numeric_limits<double>::quiet_NaN()); msg->duration =
  ...
  //   ASSERT_EQ((*(controller_->input_ref_.readFromRT()))->displacements[0],
  //   TEST_DISPLACEMENT); ASSERT_EQ(
  //     controller_->update_reference_from_subscribers(),
  //     controller_interface::return_type::OK);
  //   ASSERT_EQ(
  //     controller_->update_and_write_commands(
  ...
  //     controller_interface::return_type::OK);
  //
  //   EXPECT_EQ(*(controller_->control_mode_.readFromRT()),
  //   control_mode_type::SLOW); EXPECT_EQ(joint_command_values_[STATE_MY_ITFS],
  //   111); ASSERT_TRUE(std::isnan(controller_->reference_interfaces_[0])); for
  //   (const auto & interface : controller_->reference_interfaces_)
  //   {
  //     EXPECT_TRUE(std::isnan(interface));
  //   }
  //
  //   std::shared_ptr<ControllerReferenceMsg> msg_2 =
  //   std::make_shared<ControllerReferenceMsg>(); msg_2->header.stamp =
  //   controller_->get_node()->now() - rclcpp::Duration::from_seconds(0.01);
  //   msg_2->joint_names = joint_names_;
  //   msg_2->displacements.resize(joint_names_.size(), TEST_DISPLACEMENT);
  //   msg_2->velocities.resize(joint_names_.size(),
  //   std::numeric_limits<double>::quiet_NaN()); msg_2->duration =
  //   std::numeric_limits<double>::quiet_NaN();
  ...
  //   EXPECT_EQ(*(controller_->control_mode_.readFromRT()),
  //   control_mode_type::SLOW); EXPECT_EQ(joint_command_values_[STATE_MY_ITFS],
  //   TEST_DISPLACEMENT / 4); EXPECT_NE(joint_command_values_[STATE_MY_ITFS],
  //   111);
  //   ASSERT_EQ((*(controller_->input_ref_.readFromRT()))->displacements[0],
  //   TEST_DISPLACEMENT / 2); for (const auto & interface :
...
TEST_F(AckermannSteeringControllerTest, test_update_logic) {
  ...
  //   std::shared_ptr<ControllerReferenceMsg> msg_2 =
  //   std::make_shared<ControllerReferenceMsg>(); msg_2->header.stamp =
  //   controller_->get_node()->now(); msg_2->joint_names = joint_names_;
  //   msg_2->displacements.resize(joint_names_.size(), TEST_DISPLACEMENT);
  //   msg_2->velocities.resize(joint_names_.size(),
  //   std::numeric_limits<double>::quiet_NaN()); msg_2->duration =
  //   std::numeric_limits<double>::quiet_NaN();
...
TEST_F(AckermannSteeringControllerTest, test_ref_timeout_zero_for_update) {
  ...
  msg->header.stamp =
      controller_->get_node()->now() - rclcpp::Duration::from_seconds(0.0);
