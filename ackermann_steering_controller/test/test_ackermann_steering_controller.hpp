...
using ControllerStateMsgOdom = ackermann_steering_controller::
    AckermannSteeringController::ControllerStateMsgOdom;
using ControllerStateMsgTf = ackermann_steering_controller::
    AckermannSteeringController::ControllerStateMsgTf;
using ControllerReferenceMsg = ackermann_steering_controller::
    AckermannSteeringController::ControllerReferenceMsg;
...
class TestableAckermannSteeringController
  ...
  FRIEND_TEST(AckermannSteeringControllerTest,
  ...
  FRIEND_TEST(AckermannSteeringControllerTest, test_update_logic_chainable);
  FRIEND_TEST(AckermannSteeringControllerTest,
              test_ref_timeout_zero_for_update);
  FRIEND_TEST(AckermannSteeringControllerTest,
              test_ref_timeout_zero_for_reference_callback);

 public:
  controller_interface::CallbackReturn on_configure(
      const rclcpp_lifecycle::State& previous_state) override {
    auto ret = ackermann_steering_controller::AckermannSteeringController::
  ...
  controller_interface::CallbackReturn on_activate(
    ...
    return ackermann_steering_controller::AckermannSteeringController::
        on_activate(previous_state);
  }

  /**
   * @brief wait_for_command blocks until a new ControllerReferenceMsg is
   * received. Requires that the executor is not spinned elsewhere between the
   *  message publication and the call to this function.
   *
   * @return true if new ControllerReferenceMsg msg was received, false if
   * timeout.
   */
  bool wait_for_command(rclcpp::Executor& executor,
                        rclcpp::WaitSet& subscriber_wait_set,
                        const std::chrono::milliseconds& timeout =
                            std::chrono::milliseconds{500}) {
    bool success = subscriber_wait_set.wait(timeout).kind() ==
  ...
  bool wait_for_commands(rclcpp::Executor& executor,
                         const std::chrono::milliseconds& timeout =
                             std::chrono::milliseconds{500}) {
    return wait_for_command(executor, ref_subscriber_wait_set_, timeout);
  }

  // TODO(anyone): add implementation of any methods of your controller is
  // needed
...
// We are using template class here for easier reuse of Fixture in
// specializations of controllers
...
class AckermannSteeringControllerFixture : public ::testing::Test {
 ...
 protected:
  void SetUpController(const std::string controller_name =
    ...
    ASSERT_EQ(controller_->init(controller_name),
    ...
    command_itfs_.emplace_back(hardware_interface::CommandInterface(
        rear_wheel_name, hardware_interface::HW_IF_VELOCITY,
        &joint_command_values_[0]));
    command_ifs.emplace_back(command_itfs_.back());

    command_itfs_.emplace_back(hardware_interface::CommandInterface(
        front_steer_name, hardware_interface::HW_IF_VELOCITY,
    ...
    state_itfs_.emplace_back(hardware_interface::StateInterface(
        rear_wheel_name, hardware_interface::HW_IF_POSITION,
        &joint_state_values_[0]));
    state_ifs.emplace_back(state_itfs_.back());

    state_itfs_.emplace_back(hardware_interface::StateInterface(
        front_steer_name, hardware_interface::HW_IF_POSITION,
        &joint_state_values_[1]));
    state_ifs.emplace_back(state_itfs_.back());
  ...
  void subscribe_and_get_messages(ControllerStateMsgOdom& msg_odom) {
    ...
    auto subscription_odom =
        test_subscription_node.create_subscription<ControllerStateMsgOdom>(
            "/test_ackermann_steering_controller/odometry", 10,
            subs_callback_odom);

    // call update to publish the test value
    ASSERT_EQ(controller_->update_reference_from_subscribers(
                  controller_->get_node()->now(),
                  rclcpp::Duration::from_seconds(0.01)),
              controller_interface::return_type::OK);
    ASSERT_EQ(controller_->update_and_write_commands(
                  controller_->get_node()->now(),
                  rclcpp::Duration::from_seconds(0.01)),
              controller_interface::return_type::OK);

    // call update to publish the test value
    // since update doesn't guarantee a published message, republish until
    // received
    ...
    while (max_sub_check_loop_count--) {
      ...
      if (wait_set.wait(std::chrono::milliseconds(2)).kind() ==
          rclcpp::WaitResultKind::Ready) {
        break;
      }
    }
    ...
    ASSERT_GE(max_sub_check_loop_count, 0)
        << "Test was unable to publish a message through "
  ...
  void publish_commands(const rclcpp::Time& stamp,
                        const double& twist_linear_x = 0.45,
                        const double& twist_angular_z = 0.0) {
    auto wait_for_topic = [&](const auto topic_name) {
      size_t wait_count = 0;
      while (command_publisher_node_->count_subscribers(topic_name) == 0) {
        if (wait_count >= 5) {
          auto error_msg = std::string("publishing to ") + topic_name +
