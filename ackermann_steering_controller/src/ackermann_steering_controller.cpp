...
const char* AckermannSteeringController::feedback_type() const {
  return params_.position_feedback ? hardware_interface::HW_IF_POSITION
...
controller_interface::CallbackReturn AckermannSteeringController::on_init() {
  try {
    param_listener_ =
        std::make_shared<ackermann_steering_controller::ParamListener>(
            get_node());
...
controller_interface::CallbackReturn AckermannSteeringController::on_configure(
  ...
  const double wheel_radius =
      params_.wheel_radius_multiplier * params_.wheel_radius;
  ...
  ref_subscriber_ = get_node()->create_subscription<ControllerReferenceMsg>(
      ...
      std::bind(&AckermannSteeringController::reference_callback, this,
                std::placeholders::_1));
  ...
  try {
    ...
    rt_odom_state_publisher_ =
        std::make_unique<ControllerStatePublisherOdom>(odom_s_publisher_);
  } catch (const std::exception& e) {
    fprintf(stderr,
...
AckermannSteeringController::command_interface_configuration() const {
  ...
  command_interfaces_config.names.push_back(params_.rear_wheel_name + "/" +
                                            hardware_interface::HW_IF_VELOCITY);
  command_interfaces_config.names.push_back(params_.front_steer_name + "/" +
...
AckermannSteeringController::state_interface_configuration() const {
  ...
  state_interfaces_config.names.push_back(params_.rear_wheel_name + "/" +
                                          feedback_type());
  state_interfaces_config.names.push_back(params_.front_steer_name + "/" +
                                          hardware_interface::HW_IF_POSITION);
...
AckermannSteeringController::update_reference_from_subscribers(
    const rclcpp::Time& time, const rclcpp::Duration& /*period*/) {
  auto current_ref = *(input_ref_.readFromRT());
  const auto age_of_last_command = time - (current_ref)->header.stamp;

  // TODO(anyone): depending on number of interfaces, use definitions, e.g.,
  // `CMD_MY_ITFS`, instead of a loop send message only if there is no timeout
...
controller_interface::return_type
AckermannSteeringController::update_and_write_commands(
    const rclcpp::Time& time, const rclcpp::Duration& /*period*/) {
  if (params_.open_loop) {
    odometry_.updateOpenLoop(last_linear_velocity_, last_angular_velocity_,
                             time);
  } else {
    if (params_.position_feedback) {
      const double wheel_position = state_interfaces_[0].get_value();
      const double steer_position =
      ...
      odometry_.update(wheel_position, steer_position, wheel_vel,
                       params_.position_feedback, time);
  ...
  command_interfaces_[0].set_value(last_linear_velocity_ /
